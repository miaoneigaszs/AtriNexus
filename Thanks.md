# 致谢 / Acknowledgments

<div align="center">

**感谢所有让 AtriNexus 成为可能的人们与项目**

# AtriNexus

[![GitHub Stars](https://img.shields.io/github/stars/miaoneigaszs/AtriNexus?color=ff69b4&style=flat-square)](https://github.com/miaoneigaszs/AtriNexus/stargazers)
[![GitHub Forks](https://img.shields.io/github/forks/miaoneigaszs/AtriNexus?color=9c27b0&style=flat-square)](https://github.com/miaoneigaszs/AtriNexus/network/members)
[![License](https://img.shields.io/github/license/miaoneigaszs/AtriNexus?color=03a9f4&style=flat-square)](LICENSE)
[![Version](https://img.shields.io/badge/version-1.4.3.2-ff69b4?style=flat-square)](version.json)


</div>

---

## 🌟 核心支持

### AI 模型服务提供商

感谢以下平台提供强大的 AI 能力支持：

| 服务商 | 说明 |
|--------|------|
| **[DeepSeek](https://platform.deepseek.com/)** | 深度求索公司推出的 AI 大模型，本项目核心对话能力来源 |
| **[阿里云百炼](https://bailian.console.aliyun.com/)** | 阿里云大模型服务平台，提供图像识别等多模态能力 |
| **[硅基流动](https://cloud.siliconflow.cn/)** | AI 基础设施服务平台，提供意图识别、Embedding 等关键能力 |
| **[Tavily](https://tavily.com/)** | AI 搜索 API 平台，提供实时网络搜索能力 |

---

## 💻 技术栈致谢

### 核心开源框架与库

本项目站在巨人的肩膀上，感谢以下优秀的开源项目：

#### 🤖 AI 与 NLP
- **[OpenAI Python](https://github.com/openai/openai-python)** - OpenAI API 官方 Python 客户端
- **[LangChain](https://github.com/langchain-ai/langchain)** - 大语言模型应用开发框架
- **[Transformers](https://github.com/huggingface/transformers)** - Hugging Face 预训练模型库

#### 🌐 Web 框架
- **[Flask](https://github.com/pallets/flask)** - 轻量级 Web 应用框架
- **[FastAPI](https://github.com/tiangolo/fastapi)** - 现代 Web 框架
- **[Uvicorn](https://github.com/encode/uvicorn)** - ASGI 服务器

#### 💾 数据库与存储
- **[SQLAlchemy](https://github.com/sqlalchemy/sqlalchemy)** - Python SQL 工具包和 ORM
- **[SQLite](https://www.sqlite.org/)** - 嵌入式数据库引擎
- **[ChromaDB](https://github.com/chroma-core/chroma)** - 向量数据库

#### 🔧 工具库
- **[Requests](https://github.com/psf/requests)** - HTTP 库
- **[Pydantic](https://github.com/pydantic/pydantic)** - 数据验证库
- **[Loguru](https://github.com/Delgan/loguru)** - 日志库
- **[Rich](https://github.com/Textualize/rich)** - 终端美化库

#### 🖼️ 图像处理
- **[Pillow](https://github.com/python-pillow/Pillow)** - Python 图像处理库

---

## 👥 贡献者

感谢所有为项目做出贡献的开发者：

<a href="https://github.com/AtriNexus/AtriNexus/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=AtriNexus/AtriNexus" />
</a>

---

## 📖 灵感来源

项目受到以下优秀项目的启发：

- **[LangChain](https://github.com/langchain-ai/langchain)** - AI 应用开发范式
- **[QChatGPT](https://github.com/RockChinQ/QChatGPT)** - QQ 机器人架构参考

---

## 🙏 特别感谢

### 致所有用户

感谢所有使用本项目并提供反馈的用户，你们的建议帮助项目不断改进。

### 致开发者社区

感谢开源社区的每一位贡献者，你们的努力让技术世界更加美好。

---

...
async def metrics_middleware(request: Request, call_next):
    """请求指标收集中间件"""
    from src.platform_core.metrics import record_http_request
...
async def health_check():
    """增强版健康检查"""
    from src.platform_core.health_check import get_health_report
...
async def metrics():
    """Prometheus 指标端点"""
    from src.platform_core.metrics import get_metrics_output, get_metrics_content_type
    return Response(
...
# ========== 注册路由 ==========
from src.ingress.routers import (
    callback_router,
    knowledge_router,
    memory_router,
    config_router,
...
def start_server(host: str = "0.0.0.0", port: int = 8000):
    ...
    # 启动定时任务调度器
    from src.ingress.scheduler import init_scheduler
    init_scheduler()
